...
de_directory <- paste0("/nfs/turbo/sph-colacino/aguilada/scRNAseq/analysis/integrated/",analysis,"/FPR_0.0_MAD/stats/DESeq2/results/")
...
directory_fgsea = paste0("/nfs/turbo/sph-colacino/aguilada/scRNAseq/analysis/integrated/",analysis,"/FPR_0.0_MAD/stats/DESeq2/GSEA/fGSEA/")

dirs = c(data_dir, directory_fgsea)
...
gene_id = read.csv('/nfs/turbo/sph-colacino/aguilada/scRNAseq/analysis/3_weeks/annotation/ensembl_and_entrez_gene_ids_and_human_homologs.csv')
...
to_genelist = function(res_tbl, species_human=F) {
  if(!species_human) {
    genelist = res_tbl %>%
      ...
      na.omit() %>%
      # group_by(hsapiens_homolog_associated_gene_name) %>%
      dplyr::arrange(desc(ranking_metric)) %>%
      distinct() %>%
      deframe()
  } else {
    genelist = res_tbl %>%
...
tidysave_fgsea = function(fgseaRes, pathway_name, celltype, simple_tf=F, time,
                          directory=directory_fgsea) {
  ...
  fgseaResTidy = fgseaRes %>%
    ...
    dplyr::mutate(leadingEdge2 = leadingEdge) %>% # for saving to csv
    rowwise() %>% 
    dplyr::mutate_at(c('leadingEdge2'), ~paste(unlist(.), collapse = ',')) %>%
    dplyr::select(!leadingEdge) %>%
    dplyr::rename('leadingEdge' = leadingEdge2)
  ...
  write.csv(x=fgseaResTidy,
            file = paste0(directory,celltype,'_',time,ifelse(simple_tf,'_simple','_multilevel'),
...
plot_fgsea_nes = function(fgsea_nes, celltype, pathways, pathway_name,time,
  ...
  ggsave(paste0(directory,celltype,'_',time,ifelse(simple_tf,'_simple','_multilevel'),
...
plot_fgsea_nes_dotplot = function(fgsea_nes, celltype, pathways,pathway_name, time,
                                  padj_cutoff=0.25, simple_tf=F, directory=directory_figs) {
...
msigdb.mm = qs_read('/nfs/turbo/sph-colacino/aguilada/scRNAseq/analysis/msigdb.mm.qs',nthreads=nthreads)
...
mpt_mm = gson::read.gmt('/nfs/turbo/sph-colacino/aguilada/scRNAseq/analysis/m5.mpt.v2024.1.Mm.symbols.gmt')
mpt_mm = split(mpt_mm$gene, mpt_mm$term)
ccca_hs = gson::read.gmt('/nfs/turbo/sph-colacino/aguilada/scRNAseq/analysis/c4.3ca.v2024.1.Hs.symbols.gmt')
ccca_hs$gene = str_to_title(ccca_hs$gene)
ccca_hs = split(ccca_hs$gene, ccca_hs$term)
kegg_med_hs = gson::read.gmt('/nfs/turbo/sph-colacino/aguilada/scRNAseq/analysis/c2.cp.kegg_medicus.v2024.1.Hs.symbols.gmt')
...
for (i in 1:length(de_results)) {
  ...
  for (k in 1:length(pathway_names)) {
    ...
    if (length(fgseaRes$padj) > 0) {
      ...
      collapsedPathways = collapsePathways(fgseaRes=fgseaRes[order(pval)][padj < 0.01], 
