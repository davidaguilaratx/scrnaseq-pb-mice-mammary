...
LabelPoints(plot=p, points = top25, repel = TRUE)+
  ...
  theme_classic2()
ggsave('figures/clustering_rm29and32_regress_ccdiffandmt/top25_variable_genes_seurat_phase.png', dpi=320, width=20, height= 10)
...
saveRDS(seurat_pca, 'data/seurat_phase_rm29and32_regressout_cc.diffandmt.rds')
...
DimPlot(seurat_pca,
  ...
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1),
        ...
        axis.text = element_text(size=12),
        title = element_text(size=18))
ggsave('figures/clustering_rm29and32_regress_ccdiffandmt/PCA_by_cellcycle_phase_allgenes_.png', dpi=320, width=20, height= 10)
...
DimPlot(seurat_pca,
  ...
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1),
        ...
        axis.text = element_text(size=12),
        title = element_text(size=18))
ggsave('figures/clustering_rm29and32_regress_ccdiffandmt//PCA_by_mitochondrial_expression_quartile.png', dpi=320, width=20, height= 10)
...
DimPlot(seurat_pca,
  ...
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1),
        ...
        axis.text = element_text(size=12),
        title = element_text(size=18))
ggsave('figures/clustering_rm29and32_regress_ccdiffandmt/PCA_by_condition.png', dpi=320, width=20, height= 10)
...
saveRDS(seurat_merged_sct, 'data/seurat_merged_sct_rm29and32_regressout_cc.diffandmt.rds')
...
saveRDS(sct_results, 'figures/clustering_rm29and32_regress_ccdiffandmt/sct_results_rm29and32_5xMADfiltering_regressout_cc.diffandmt.rds')
...
# VariableFeaturePlot() won't work for merged object after SCTtransform.
...
lapply(seq_along(split_seurat), function(x,n,i) {
  ...
  LabelPoints(plot=p, points = top25_sct, repel = TRUE)+
    ...
    theme_classic2()
  ggsave(paste0('figures/clustering_rm29and32_regress_ccdiffandmt/top25_variable_genes_seurat_phase_',n[[i]],'.png'), dpi=320, width=20, height= 10)
...
ggsave('figures/clustering_rm29and32_regress_ccdiffandmt/sct/PCA_loadings_sct.png', dpi=320, width=20, height= 15)
...
## clusters after running with dims=1:50, removed low quality samples 29 and 31, and regressed out cc.differecne and percent.mt
...
saveRDS(seurat_umap_sct, 'data/seurat_umap_sct_rm29and32_regressout_cc.diffandmt.rds')
...
# check out resolutions 0.1, 0.2, 0.6, 0.8, 1.4
...
## plot regular umap, as well as by sample, condition, cell cycle, and mitochondrial expression quartile for resolution SCT_snn_res.0.6 ----
plot_umaps(seurat_umap=seurat_umap_sct, resolution='SCT_snn_res.0.6', directory='figures/clustering_rm29and32_regress_ccdiffandmt/sct/')

# clustering QC at SCT_snn_res.0.6 resolution ----
umap_clustering_qc(seurat_umap=seurat_umap_sct, resolution='SCT_snn_res.0.6', directory='figures/clustering_rm29and32_regress_ccdiffandmt/sct/')

## plot PCs on umap for resolution SCT_snn_res.0.6 ----
pcs_umap_overlay(seurat_umap=seurat_umap_sct, resolution='SCT_snn_res.0.6', directory='figures/clustering_rm29and32_regress_ccdiffandmt/sct/')

## overlay umap and create dotplot with gene markers for resolution SCT_snn_res.0.6. ----
# takes a while to run
plot_gene_markers(seurat_umap=seurat_umap_sct, resolution='SCT_snn_res.0.6', directory='figures/clustering_rm29and32_regress_ccdiffandmt/sct/')
...
#### annotate clusters ----
Idents(seurat_umap_sct) = 'SCT_snn_res.1.4' # set cluster #s as ident
...
# save markers
# write.csv(ann.pos.markers,'annotation/annotated_pos_markers_rm29and32_regressout_ccdiffandmt.csv',row.names=FALSE)

ann.pos.markers = read.csv('annotation/annotated_pos_markers_rm29and32_regressout_ccdiffandmt.csv')
...
conserved.pos.markers = map_dfr(c(0:29), get_conserved) # 31 clusters at resolution 1.4
...
# save markers
write.csv(conserved.pos.markers,'annotation/conserved_pos_markers_rm29and32_regressout_ccdiffandmt.csv',row.names=FALSE)
...
topc28 = conserved.pos.markers[conserved.pos.markers$cluster_id == 29,] %>%
...
rm(combined_seurat) # clean up RAM
gc() 


# Rename cluster #'s to cell type annotations ----
Idents(seurat_umap_sct) = seurat_umap_sct$SCT_snn_res.1.4
...
saveRDS(seurat_umap_sct, 'data/annotated_seurat_rm29and32_regressout_ccdiffandmt.rds')
