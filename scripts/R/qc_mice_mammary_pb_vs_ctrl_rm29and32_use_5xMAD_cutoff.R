...
mads_df = data.frame(median_gene=numeric(12), MAD_gene=numeric(12), plus_5MAD_gene=numeric(12), minus_5MAD_gene=numeric(12),
                     ...
                     median_mt=numeric(12), MAD_mt=numeric(12), plus_5MAD_mt=numeric(12))
for (i in 1:12) {
  ...
  plus_5MAD_mt = median_mt + 5*MAD_mt
...
plus_5MAD_mt = mean(mads_df$plus_5MAD_mt)

ggplot(metadata,aes(x=nCount_RNA,y=nFeature_RNA,color=percent.mt))+
  geom_point(size=0.5)+
  DarkTheme()+
  labs(title='Count depth vs # of genes',
       subtitle=paste0('colored by percent mitochondrial (mt) counts >', plus_5MAD_mt, ' (5xMAD)'),
       x='# of UMI counts)',
       y='# of genes)',
       color='% mt counts')+
  scale_x_log10(n.breaks=8, labels=comma_format())+
  ...
  scale_color_gradientn(colors=viridis_plasma_light_high, limits = c(plus_5MAD_mt, NA), na.value ='#292a2d')
...
length(filtered_seurat$nCount_RNA[filtered_seurat$sample == 'ctrl24'])
# 1694
1720-1694 # 26 cells filtered out of sample ctrl24
...
length(filtered_seurat$nCount_RNA[filtered_seurat$sample == 'pb31'])
# 1990
2096 - 1990 # 106 cells filtered out of sample pb31
...
## cell-level filtering of all remaining samples by 5 times the median absolute deviation (5xMAD)----
ncells = length(filtered_seurat$orig.ident) # for tracking # cells before filtering

# split by sample and filter by 5xMAD
split_filtered_seurat = SplitObject(filtered_seurat, split.by = "orig.ident")

split_filtered_seurat_byMAD = lapply(split_filtered_seurat, function(sample) {
  
  c = median(sample$nCount_RNA) + 5*mad(sample$nCount_RNA)
  
  g = median(sample$nFeature_RNA) + 5*mad(sample$nFeature_RNA)
  
  # m = median(sample$percent.mt) + 5*mad(sample$percent.mt)
  
  sample = subset(sample, subset=(nCount_RNA <= c) &
                   (nFeature_RNA <= g) & (percent.mt <= 10) & (log10GenesPerUMI >= 0.8))
  
})

# merge filtered samples into one object
filtered_seurat =  merge(split_filtered_seurat_byMAD[[1]], split_filtered_seurat_byMAD[-1], merge.data=TRUE,merge.dr=FALSE) 
...
# 42467 cells left after cell-level filtering
# 1715 cells removed after cell-level filtering

rm(split_filtered_seurat, split_filtered_seurat_byMAD) # free up RAM
gc() 
...
saveRDS(filtered_seurat, 'data/filtered_seurat_rm29and32_5xMADfiltering.rds')
filtered_metadata = filtered_seurat@meta.data
saveRDS(filtered_metadata, 'data/filtered_metadata_rm29and32_5xMADfiltering.rds')
...
# 1715 total cells removed after cell and gene-level filtering
# 42467 cells after filtering
...
plot_grid(p1, p2)
ggsave('figures/qc/ncells_before_and_after_filtering_rm29and32_5xMADfiltering.png', dpi=320, width=20, height= 10)
...
p1 = ggplot(metadata,aes(x=nCount_RNA,y=nFeature_RNA,color=percent.mt))+
  ...
  DarkTheme()+
  labs(title='Count depth vs # of genes',
       subtitle=paste0('colored by percent mitochondrial (mt) counts >', 10),
       x='# of UMI counts (log scale)',
       y='# of genes (log scale)',
       color='% mt counts')+
  scale_x_log10(n.breaks=8, labels=comma_format())+
  ...
  NoGrid()+
  stat_smooth(method=lm,linewidth=0.5, colour='#61e4e8')+
  scale_color_gradientn(colors=viridis_plasma_light_high, limits = c(10, NA), na.value ='#4b4d53')
...
plot_grid(p1, p2)
ggsave('figures/qc/ncounts_vs_ngenes_coloredby_percentmt_before_and_after_filtering_rm29and32_5xMADfiltering.png', dpi=320, width=20, height= 10)
...
i = WhichCells(subset(merged_seurat, idents=c('count29','count32'),invert=TRUE)
               , expression=(nCount_RNA >= c) | (nFeature_RNA >= g) | 
                 ((percent.mt <= 10) & (percent.mt >=4.72)) | (log10GenesPerUMI <= 0.8))
length(i) # 1154
# 1805

j = WhichCells(subset(merged_seurat, idents=c('count29','count32'),invert=TRUE),
               expression=(nCount_RNA <= c) & (nFeature_RNA <= g) & 
                 (percent.mt <= m) & (log10GenesPerUMI >= 0.8))
length(j)
...
total_cells_filtered = vector(mode='numeric',length=10) 
cells_filtered_by_sample = vector(mode='numeric',length=10)
for (i in seq_along(samples)) {
  ...
  t = WhichCells(subset(merged_seurat,idents=samples[i]), expression=(nCount_RNA >= c) |
                   (nFeature_RNA >= g) |((percent.mt >= 10) | percent.mt >= 10) | (log10GenesPerUMI <= 0.8))
...
joined = JoinLayers(filtered_seurat)
