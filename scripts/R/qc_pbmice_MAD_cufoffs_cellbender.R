...
default_dir_turbo = "//sph-colacino-win.turbo.storage.umich.edu/sph-colacino/aguilada/scRNAseq/analysis/"
...
for (selected_timepoint in names(timepoints)) {
  ...
  for (i in timepoints[[selected_timepoint]]) {
    ...
    fltrd_seurat_obj <- RunPCA(fltrd_seurat_obj, npcs=50,verbose = T)
    fltrd_seurat_obj <- FindNeighbors(fltrd_seurat_obj, dims = 1:50, verbose = T)
    fltrd_seurat_obj <- FindClusters(fltrd_seurat_obj, resolution = 0.8, verbose = T)
